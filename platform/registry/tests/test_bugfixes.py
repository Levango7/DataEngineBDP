...
class TestDeploymentModelValidation:
    ...
    def test_create_deployment_unknown_model_returns_404_model_not_found(
    ...
    def test_create_deployment_unknown_version_returns_404(
        self, client, registered_model, sample_deploy_request
    ):
        """模型存在但指定版本不存在也应 404 model_not_found."""
        payload = sample_deploy_request.model_copy(
...
class TestEventLoopNotBlocked:
    ...
    def test_real_mode_docker_run_does_not_block_event_loop(
        self, monkeypatch
    ):
        """docker run 慢调用期间事件循环仍可响应其他请求."""

        def fake_docker_run(cmd, capture_output=False, text=False,
                            timeout=None, **kwargs):
            time.sleep(0.5)
        ...
        async def scenario():
            ...
            async with httpx.AsyncClient(
                ...
                slow = asyncio.create_task(
                    ac.post("/api/v1/registry/deployments", json=payload)
                )
    ...
    def test_real_mode_docker_stop_does_not_block_event_loop(
        self, monkeypatch
    ):
        ...
        def fake_docker_stop(cmd, capture_output=False, text=False,
                             timeout=None, **kwargs):
            time.sleep(0.5)
        ...
        record = manager.deploy(
            DeployRequest(
                modelName="m", version="0.1.0", port=8000,
                tenantId="tenant-001",
            )
        )
        ...
        async def scenario():
            ...
            async with httpx.AsyncClient(
                ...
                slow = asyncio.create_task(
                    ac.delete(
                        f"/api/v1/registry/deployments/{record.deploymentId}"
                    )
                )
                await asyncio.sleep(0.1)
                start = time.perf_counter()
