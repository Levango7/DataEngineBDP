"""MIRRORED FILE — 此文件在 llmops/ml-platform/nl2sql/evaluation/knowledge-engine/asset-exchange/open-api-catalog/business-portal/registry/batch-pipeline/model-finetuning(loop)/industry-templates/model-finetuning(app) 十三处保持逐字节一致。
修改任一幅本必须同步其余十二处，CI 由 scripts/check-mirrored-jwt-auth.sh 强制。
