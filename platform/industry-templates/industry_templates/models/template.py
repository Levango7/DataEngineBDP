...
class TemplateParameter(BaseModel):
    ...
    enumOptions: Optional[list[str]] = Field(
        default=None, description="枚举可选值"
    )
...
class DataFlowNode(BaseModel):
    ...
    layer: Optional[str] = Field(
        default=None, description="数据分层: ods/dwd/dws/ads"
    )
    ...
    config: dict[str, Any] = Field(
        default_factory=dict, description="节点配置（SQL/参数等）"
    )
...
class DataFlowConfig(BaseModel):
    ...
    nodes: list[DataFlowNode] = Field(
        default_factory=list, description="数据流节点列表"
    )
    schedule: Optional[str] = Field(
        default=None, description="调度周期（cron 表达式）"
    )
...
class ComputeLogicStep(BaseModel):
    ...
    params: dict[str, Any] = Field(
        default_factory=dict, description="步骤参数"
    )
...
class ComputeLogicConfig(BaseModel):
    ...
    steps: list[ComputeLogicStep] = Field(
        default_factory=list, description="计算步骤列表"
    )
...
class VisualizationPanel(BaseModel):
    ...
    config: dict[str, Any] = Field(
        default_factory=dict, description="ECharts option 配置"
    )
...
class VisualizationConfig(BaseModel):
    ...
    panels: list[VisualizationPanel] = Field(
...
class Template(TimestampMixin):
    ...
    dataFlow: DataFlowConfig = Field(
        default_factory=DataFlowConfig, description="数据流配置"
    )
    computeLogic: ComputeLogicConfig = Field(
        default_factory=ComputeLogicConfig, description="计算逻辑配置"
    )
    visualization: VisualizationConfig = Field(
        default_factory=VisualizationConfig, description="可视化配置"
    )
...
class DeploymentRequest(BaseModel):
    ...
    namespace: Optional[str] = Field(
        default=None, description="K8s namespace（不传则用租户默认）"
    )
    values: dict[str, Any] = Field(
        default_factory=dict, description="参数取值（注入 values.yaml）"
    )
...
class DeploymentRecord(TimestampMixin):
    ...
    status: DeploymentStatus = Field(
        default=DeploymentStatus.PENDING, description="部署状态"
    )
    renderedValues: dict[str, Any] = Field(
        default_factory=dict, description="渲染后的 values"
    )
    errorMessage: Optional[str] = Field(
        default=None, description="失败原因"
    )
    jobRunId: Optional[str] = Field(
        default=None, description="首次作业运行 ID"
    )
    dashboardSnapshotUrl: Optional[str] = Field(
        default=None, description="仪表盘快照 URL"
    )
...
class TemplatePreview(BaseModel):
    ...
    architecture: dict[str, Any] = Field(
        default_factory=dict, description="架构图（节点+边）"
    )
    parameterSummary: list[dict[str, Any]] = Field(
        default_factory=list, description="参数摘要"
    )
    stats: dict[str, int] = Field(
        default_factory=dict, description="统计：节点数/步骤数/面板数"
    )