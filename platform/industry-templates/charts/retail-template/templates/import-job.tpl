...
用途：部署一个 Job，拉取 ConfigMap 中的模板资产，依次导入到 Doris/DolphinScheduler/Superset/Keycloak/tag-engine
*/ -}}
      ...
      containers:
        - name: template-importer
          ...
          env:
            ...
            - name: TAG_ENGINE_HOST
              value: {{ .Values.target.tagEngine.host | quote }}
            - name: TAG_ENGINE_PORT
              value: {{ .Values.target.tagEngine.port | quote }}
            - name: TAG_ENGINE_TOKEN
              valueFrom:
                secretKeyRef:
                  name: {{ .Values.target.tagEngine.tokenSecret.name }}
                  key: {{ .Values.target.tagEngine.tokenSecret.key }}
            ...
            - name: TAG_ENGINE_SYNC_ENABLED
              value: {{ .Values.tagEngineSync.enabled | quote }}
            - name: TAG_ENGINE_ROLLBACK_ON_FAILURE
              value: {{ .Values.tagEngineSync.rollbackOnFailure | quote }}
          command:
            - /bin/sh
            - -c
            - |
              ...
              if [ "${RBAC_SYNC_ENABLED}" = "true" ]; then
                ...
                echo "[4/5] RBAC 同步已禁用，跳过"
              fi
              if [ "${TAG_ENGINE_SYNC_ENABLED}" = "true" ]; then
                echo "[5/5] 导入标签引擎配置..."
                curl -X POST "http://${TAG_ENGINE_HOST}:${TAG_ENGINE_PORT}/api/v1/tag-config/import" \
                  -H "Authorization: Bearer ${TAG_ENGINE_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d @/templates/tag-engine/tag-engine-config.yaml
                echo "[5/5] 标签引擎配置导入完成"
              else
                echo "[5/5] 标签引擎同步已禁用，跳过"
              fi
              echo "=========================================="
              echo "零售行业模板导入完成"
              echo "=========================================="
