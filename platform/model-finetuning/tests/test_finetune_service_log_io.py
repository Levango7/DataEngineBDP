...
def _register_running_task(
    service: FinetuneService, tmp_path, log_path: str, task_id: str
):
    ...
    adapter = get_adapter(
        request.config.framework, workDir=service.workDir, mockMode=True
    )
    with service._lock:
...
def _install_to_thread_spy(monkeypatch) -> list[str]:
    ...
    monkeypatch.setattr(
        finetune_service_module.asyncio, "to_thread", _to_thread
    )
    return calls
...
class TestTailFileUtf8Boundary:
    ...
    def test_tail_only_reads_bounded_bytes_with_intact_chinese(
...
class TestAsyncLogIO:
    def test_get_logs_offloads_slow_tail_read_to_thread(
        self, tmp_path, monkeypatch
    ):
        service = FinetuneService(
            workDir=str(tmp_path / "work"), mockMode=True
        )
        log_path = str(tmp_path / "big.log")
        _write_big_log(log_path, filler_mb=10)
        task = _register_running_task(
            service, tmp_path, log_path, "ft-logiotest0001"
        )
    ...
    def test_refresh_marks_succeeded_via_to_thread_on_big_log(
        self, tmp_path, monkeypatch
    ):
        service = FinetuneService(
            workDir=str(tmp_path / "work"), mockMode=True
        ...
        assert updated.outputModelPath == os.path.join(
            task.request.outputDir, task.taskId
        )

    def test_refresh_without_marker_or_missing_log_keeps_running(
        self, tmp_path, monkeypatch
    ):
        service = FinetuneService(
            workDir=str(tmp_path / "work"), mockMode=True
        ...
        task = _register_running_task(
            service, tmp_path, str(running_log), "ft-logiorun00001"
        )
        updated = asyncio.run(service.refresh_task_status(task.taskId))
        assert updated.status == TaskStatus.RUNNING

        missing_task = _register_running_task(
            service, tmp_path, str(tmp_path / "no-such-file.log"),
            "ft-logiomiss001",
        )
        updated_missing = asyncio.run(
            service.refresh_task_status(missing_task.taskId)
        )
