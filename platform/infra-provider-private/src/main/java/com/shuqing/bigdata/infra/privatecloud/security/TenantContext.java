package com.shuqing.bigdata.infra.privatecloud.security;

/**
 * 租户上下文持有器。
 *
 * <p>基于 {@link ThreadLocal} 在当前请求线程内传递租户 ID 与用户 ID，
 * 必须在请求结束时通过 {@link #clear()} 清理，避免线程池复用导致的
 * 租户串号与内存泄漏。</p>
 *
 * <p>典型用法：在 {@code JwtAuthFilter} 的 {@code doFilterInternal} 中
 * 解析 JWT 后调用 {@link #setTenantId(String)}，并在 finally 块中调用 {@link #clear()}。</p>
 *
