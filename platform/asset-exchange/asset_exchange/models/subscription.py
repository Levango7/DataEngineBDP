...
class SubscriptionFilter(BaseModel):
    ...
    status: Optional[SubscriptionStatus] = Field(default=None, description="按状态过滤")
...
class Subscription(TimestampMixin):
    ...
    period: Optional[str] = Field(default=None, description="订阅周期: monthly/quarterly/yearly")

    # 拉取配置（订阅模式用）
    pullConfig: dict[str, Any] = Field(default_factory=dict, description="拉取配置（如 cron、增量字段）")
