...
class Settlement(TimestampMixin):
    ...
    status: SettlementStatus = Field(default=SettlementStatus.PENDING, description="结算状态")
    # 结算金额
    totalAmount: float = Field(default=0.0, ge=0, description="结算总金额（元）")
    providerRevenue: float = Field(default=0.0, ge=0, description="提供方收益（元）")
    platformRevenue: float = Field(default=0.0, ge=0, description="平台抽成（元）")
    # 关联
    billingRecordIds: list[str] = Field(default_factory=list, description="关联计费记录 ID 列表")
    # 配置
    providerShare: float = Field(default=0.8, ge=0, le=1, description="提供方分成比例")
    platformShare: float = Field(default=0.2, ge=0, le=1, description="平台分成比例")
    # 时间
...
class SettlementFilter(BaseModel):
    ...
    status: Optional[SettlementStatus] = Field(default=None, description="按状态过滤")
...
class Allocation(TimestampMixin):
    ...
    assetId: str = Field(..., description="资产 ID")
    status: AllocationStatus = Field(default=AllocationStatus.PENDING, description="分账状态")
    # 分账明细
    providerAmount: float = Field(default=0.0, ge=0, description="提供方分账金额")
    platformAmount: float = Field(default=0.0, ge=0, description="平台分账金额")
    ...
    # 时间
    allocatedAt: Optional[datetime] = Field(default=None, description="分账完成时间")
    errorMessage: Optional[str] = Field(default=None, description="失败原因")
...
class AllocationFilter(BaseModel):
    ...
    assetId: Optional[str] = Field(default=None, description="按资产过滤")
    settlementId: Optional[str] = Field(default=None, description="按结算过滤")
    status: Optional[AllocationStatus] = Field(default=None, description="按状态过滤")
    limit: int = Field(default=100, ge=1, le=1000)
    offset: int = Field(default=0, ge=0)
...
class SettleRequest(BaseModel):
    ...
    period: Optional[str] = Field(default=None, description="计费周期，不传则取当前年月")
