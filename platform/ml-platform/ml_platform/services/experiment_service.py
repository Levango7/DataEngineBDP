...
class ExperimentService:
    ...
    async def createExperiment(self, config: ExperimentConfig) -> ExperimentInfo:
        """创建实验，返回完整信息."""
        experimentId = await self._store.create_experiment(config)
        return await self._store.get_experiment(experimentId)

    async def getExperiment(self, experimentId: str) -> ExperimentInfo:
        return await self._store.get_experiment(experimentId)

    async def listExperiments(self) -> list[ExperimentInfo]:
        return await self._store.list_experiments()

    async def deleteExperiment(self, experimentId: str) -> None:
        await self._store.delete_experiment(experimentId)

    async def logMetrics(self, experimentId: str, metrics: dict[str, float]) -> ExperimentInfo:
        await self._store.log_metrics(experimentId, metrics)
        return await self._store.get_experiment(experimentId)

    async def logParams(self, experimentId: str, params: dict) -> ExperimentInfo:
