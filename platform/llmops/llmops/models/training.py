...
class TrainingConfig(BaseModel):
    ...
    learningRate: float = Field(
        default=2e-5, gt=0, lt=1, description="学习率"
    )
    # GPU 资源（卡数）
    ...
    # 微调方法
    finetuneMethod: str = Field(
        default="lora", description="微调方法: lora/qlora/full"
    )
    # 最大序列长度
...
class TrainingJob(TimestampMixin):
    ...
    status: TrainingJobStatus = Field(default_factory=lambda: TrainingJobStatus(
        status="pending", totalEpochs=0
    ))
    # 训练开始/结束时间
...
class EvalMetrics(BaseModel):
    ...
    hallucinationRate: float = Field(
        ..., ge=0.0, le=1.0, description="幻觉率"
    )
    upliftVsBase: float = Field(
        ..., description="对比基座提升（百分点）"
    )
    # 评估数据集
