...
> - **设计模块数 49**（产品原型 §3.3）→ **自研组件数 45**（platform/ 构建文件实测）→ **矩阵实列 42**（governance/finops 合并显示）→ **独立部署单元 41**（ADR-001）→ **platform/ 目录数 39**（一级子目录）。
...
> **冻结声明**：v2.1 ~ v2.2 周期内不再新增骨架组件（skeleton / Mock-default），现有 **13 个骨架组件**统一标记为 `frozen-until-mvp`，按处置决策分类管理，避免骨架无序膨胀稀释工程资源。
>
...
> 逐组件处置决策详见 [组件成熟度矩阵 - 骨架/Mock 默认](docs/component-maturity.md#三骨架--mock-默认13-个)。
