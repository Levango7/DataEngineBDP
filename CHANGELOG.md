...
数据引擎大数据平台 v2.0.0（开发里程碑，非 GA 发布）。在 v1.0.0 基础上新增云原生与 AI 方向模块，53 个开发任务（估算工作量 755 人天）的骨架与文档交付。**本版本为框架/原型级，外部依赖（K8s/Trino/Doris/Milvus 等）多为 Mock 或未端到端联调，不适合生产使用。**
