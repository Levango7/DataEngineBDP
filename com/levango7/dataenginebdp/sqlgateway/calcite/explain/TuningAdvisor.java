...
import com.shuqing.bigdata.sqlgateway.calcite.rule.ProjectionStatistics;
import com.shuqing.bigdata.sqlgateway.calcite.rule.PushDownStatistics;
...
public class TuningAdvisor {
    ...
    private void addPushDownSuggestions(List<Suggestion> suggestions,
        ...
        // 按谓词类型分析未下推原因
        Map<com.shuqing.bigdata.sqlgateway.calcite.rule.PredicateType, int[]> typeStats =
                predStats.getTypeStats();
        for (Map.Entry<com.shuqing.bigdata.sqlgateway.calcite.rule.PredicateType, int[]> entry :
                typeStats.entrySet()) {
            int[] counts = entry.getValue();
            if (counts[0] > 0 && counts[1] == 0
                    && entry.getKey() == com.shuqing.bigdata.sqlgateway.calcite.rule.PredicateType.UNSUPPORTED) {
                suggestions.add(new Suggestion(Severity.WARN,
