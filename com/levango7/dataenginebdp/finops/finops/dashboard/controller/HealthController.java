package com.shuqing.bigdata.finops.dashboard.controller;

import com.shuqing.bigdata.common.health.controller.AbstractHealthController;
import com.shuqing.bigdata.common.health.dto.HealthResponse;
import com.shuqing.bigdata.finops.dashboard.collector.PrometheusQueryClient;
import org.springframework.beans.factory.ObjectProvider;
import org.springframework.boot.info.BuildProperties;
import org.springframework.web.bind.annotation.RestController;

import java.util.LinkedHashMap;
import java.util.Map;

/**
 * FinOps 看板健康检查控制器。
 *
 * <p>基于 {@link AbstractHealthController} 模板方法，提供三个端点：</p>
 * <ul>
 *   <li>{@code GET /api/v1/health/liveness} - 存活探针，仅检查进程存活，始终快速返回 UP。</li>
 *   <li>{@code GET /api/v1/health/readiness} - 就绪探针，探测 Prometheus 数据源连通性。</li>
 *   <li>{@code GET /api/v1/health} - 向后兼容端点，委托就绪探针。</li>
 * </ul>
 *
 * <p>readiness 通过 {@link PrometheusQueryClient#isAvailable()} 探测 Prometheus 连接，
 * 不可用时返回 DOWN 供 K8s readinessProbe 摘除流量。</p>
 *
 * <p>版本号从 {@link BuildProperties} 动态读取，未配置 build-info 时降级为 {@code "unknown"}。</p>
 *
 * @author shuqing-bigdata
 */
@RestController
public class HealthController extends AbstractHealthController {

    private final PrometheusQueryClient prometheusQueryClient;

    /**
     * 构造控制器。
     *
     * @param buildPropertiesProvider BuildProperties 可选注入提供者
     * @param prometheusQueryClient   Prometheus 查询客户端，用于探测数据源连通性
     */
    public HealthController(ObjectProvider<BuildProperties> buildPropertiesProvider,
                            PrometheusQueryClient prometheusQueryClient) {
        super(buildPropertiesProvider);
        this.prometheusQueryClient = prometheusQueryClient;
    }

    @Override
    protected String serviceName() {
        return "finops-dashboard";
    }

    @Override
    protected HealthResponse probeReadiness() {
        boolean available = prometheusQueryClient.isAvailable();
        Map<String, Object> details = new LinkedHashMap<>();
        details.put("prometheusUrl", prometheusQueryClient.getPrometheusUrl());
        if (!available) {
            details.put("prometheus", "unavailable");
            return HealthResponse.down(serviceName(), resolveVersion(), details);
        }
        return HealthResponse.up(serviceName(), resolveVersion(), details);
    }
}
