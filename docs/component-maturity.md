...
> 数据引擎大数据平台 `platform/` 全部 **44 个自研组件**（Java 22 / Go 11 / Python 11，按构建文件 pom.xml / go.mod / pyproject.toml 实测口径）的真实成熟度盘点，用于校正 README、ROADMAP 与发布物料中的能力表述。
>
...
| llm-gateway-evaluation | Python 3.11 · FastAPI（大模型网关评测子模块） | 骨架（评测脚手架） | 无持久化 | 评测指标采集与 llm-gateway 联调待完善 |
...
| karmada-federated-query | Java 17 · Spring Boot 4.1.1（跨集群联邦查询模块） | 骨架（查询编排未接真实多集群） | H2 文件 | 跨集群查询路由与结果归并需真实 Karmada 控制面环境 |
| karmada-failover | Go 1.26（含 failover/api + failover/engine 两子模块） | 骨架（故障转移引擎脚手架） | 无持久化（故障转移策略内存态） | 故障检测与切换执行需真实多集群环境验证 |
