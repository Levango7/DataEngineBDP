...
> 早期架构（v0.3 之前）采用 L0 机器供应 + L1 云原生底座层（自建 K8s）分离设计。v0.4 起 **L1 已合并入 L0**，原因：K8s 自建底座（SKE 发行版、Cilium、存储、可观测、封装层）与机器供应紧密耦合——bootstrap/内核调优/etcd 部署均依赖具体机器资源，拆为独立 L1 层导致接口冗余且层间割裂。合并后 L0 承担"机器供应 + 自建 K8s 底座 + 封装层"全部职责。**L1 编号缺位保留**（不重编号为 L0~L4），以兼容已有 43 份详细设计文档与全仓引用，L1 缺位本身即"已合并"的语义标记。详见 `design/多平台多租户大数据平台_产品原型设计_v0.4.md` §3.1.1。
