...
> 本指南描述数据引擎大数据平台在四环境（信创 / 本地数据中心 / 公有云 / 私有云）下的部署流程。部署以 SKE 交付底座（基于 kubeadm/kind 封装的 K8s）为底座，通过 Helm 编排全部 88 个 Helm Chart（全部含 templates 与 values，生产化模板 HPA/PDB/Ingress 已补齐）。
...
| 生产 - 企业版 | 8+ | 64 核 128 GB | 2 TB NVMe / 节点 |
