...
|------|--------------|--------|------|-----------|------------|
| Java (Line) | 42.26% | 85% | -42.74% | 2/22 (9.1%) | 行≥80% / 分支≥70% |
| Java (Branch) | 26.05% | 85% | -58.95% | 0/22 (0%) | 同上 |
| Go | ~30% | 85% | -55% | 0/11 (0%) | ≥70% |
...
|------|--------|--------|--------|----------|--------------------|
| Java (Line) | 42.26% | ~44% | 85% | +1.74% | 2/22 (9.1%) |
| Java (Branch) | 26.05% | ~28% | 85% | +1.95% | 0/22 (0%) |
| Go | ~30% | ~45% | 85% | +15% | 5/11 (45.5%) |
...
|------|--------|--------|
| Java 门禁覆盖范围 | 仅 encaps-layer（1 个模块） | 全量 22 个模块 |
| Go 门禁覆盖范围 | 5 个模块（catalog/vector-engine/llm-gateway/query-api/dqctl） | 全量 11 个模块 |
...
> **2026-09-01 实化提升**：Java 阈值从 35%/15% 提升至 40%/18%（Boot 4.1.1 升级后 22 模块实测，
...
- **Java**：对所有 22 个模块执行 JaCoCo 门禁，行 < 40% 或分支 < 18% 阻断 CI
- **Go**：对所有 11 个模块执行 `go test -coverprofile` + `go tool cover -func`，覆盖率 < 30% 阻断 CI
