...
| 21 | **okhttp 3.14.0 未抬版（Trivy run #1241 剩最后一项，需你裁决）**：由 `com.vesoft:client 3.8.4`（NebulaGraph 图客户端）传递带入（实证：`client-3.8.4.pom:267-269` 直接声明 okhttp 3.14.0），仅出现在 `governance/lineage-analyzer` 与 `governance/real-time-pipeline`；其余模块经华为云 SDK 用的是 4.12.0，两套版本不共存于同一模块。run #1246 复核：这是**全局仅剩的 1 条 HIGH**，与治理草案 `docs/治理闭环修复草案.md` 的 **Q2（是否保留 Nebula 分支）是同一个决策**——删分支即转绿 | 该 CVE 仍在；3->4 是跨大版本且不兼容，vesoft 客户端按 okhttp 3.x 编译，盲抬会让血缘解析与实时管线的图库访问在运行期炸；且 Nebula 默认 `enabled:false`、本仓无实例，抬版后无法验证 | 二选一：①裁决删掉 Nebula 分支（连带移除 vesoft 客户端，Trivy 转绿、无需验证）；②搭一套可连的 NebulaGraph 环境做真实连通性验证（血缘查询 + 质量规则写入），确认后再在根 pom 集中钉 4.12.0 | 其余 9 项已按修复版抬完并实测：6 个受影响 Java 模块 `mvn test` 全绿（62+87+45+798+15+13=1020 测试 0 失败），3 个 Go 模块 `go build/test` + golangci-lint 0 finding |
