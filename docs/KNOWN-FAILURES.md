...
| 21 | **okhttp 3.14.0 未抬版（Trivy run #1241 剩最后一项，需你裁决）**：由 `com.vesoft:client 3.8.4`（NebulaGraph 图客户端）传递带入，仅出现在 `governance/lineage-analyzer` 与 `governance/real-time-pipeline`；其余模块经华为云 SDK 用的是 4.12.0，两套版本不共存于同一模块 | 该 CVE 仍在；3->4 是跨大版本且不兼容，vesoft 客户端按 okhttp 3.x 编译，盲抬会让血缘解析与实时管线的图库访问在运行期炸 | 在有 NebulaGraph 可连的环境做一次真实连通性验证（血缘查询 + 质量规则写入），确认可抬后再在根 pom 集中钉 4.12.0 | 其余 9 项已按修复版抬完并实测：6 个受影响 Java 模块 `mvn test` 全绿（62+87+45+798+15+13=1020 测试 0 失败），3 个 Go 模块 `go build/test` + golangci-lint 0 finding |
