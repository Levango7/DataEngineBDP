...
| 20 | **Integration Test 剩余红已按根因分诊（#1246/#1247 逐条归并，共 51 failed / 138 errors）**：**已修三类**——①组件路径漂移 66 error（`test_finetuning.py:50`、`test_finetuning_loop.py:61,68` 仍指 `platform/model-finetuning`、`platform/registry`，而组件 2026-09-12 已移至 `design/planned/`；空目录入 `sys.path` 后 `from app.main import app` 静默导到 evaluation 的 app 包，才报出 `EVAL_DEV_MODE/JWT_SECRET`、`module 'main' has no attribute 'app'` 这类与联邦/微调无关的错。本地实测 41 passed、22 passed/3 skipped）；②fixture 名错配 33 处 `F821`（我自己 `199a4f10` 把参数改成 `api_admin_client` 却漏改函数体，12 个用例从未真正执行）；③mock 写死端口撞车 14 error（mock 用 18093/18094，与 compose 发布的 business-portal/asset-exchange 宿主端口相同，session fixture 抛 `EADDRINUSE` 拖垮整模块 → 改 `bind(0)` 由内核分配）；④e2e 缺键 8 error（`E2E_BASE_URLS` 无 `karmada`/`observability` 键但 fixture 按下标取值 → KeyError，本应 skip 的变成 error；已补 18101/18102 与有出处的健康路径）。**剩余四类，性质各不同**：A) `test_asset_exchange.py` 10 例 —— **不是拓扑问题，是未实现的第二种契约**：服务实现的是账单驱动 `POST /api/v1/settlements`（201，`settlements.py:31-48`）与 `GET /settlements?assetId=&period=`，测试断言的是按资产 `POST /api/v1/assets/{id}/settle` 且要返回 `{totalAmount, providerRevenue}` 分账体；前端与 API 契约文档对两种形状引用数均为 0，故无消费者可依，属产品缺口需裁决；B) k3s 四链路 39 例 —— `conftest.py:566,590` 明确写着 **T-05 CI 严格模式：部署失败不加 skip marker**，即这些红是仓库有意选择的结果，要绿必须在 CI 里真给一个集群（该 job `timeout-minutes: 30`，pytest 单轮已 12 分钟，加建集群需一并评估）；C) `perf/` 16 例 —— 报 `LoadResult(success_count=0 ...)` 与 429，是压测目标未接通/限流，不是阈值松紧问题；D) encaps 与 realtime_governance 约 15 例 —— 真实服务端 500/409/403/404 与 1 例 `ReadTimeout(18090)` | Integration Test 腿仍非全绿 | ①②③④ 已提交 `38dc455f` 并触发 #1248 复测；A 需产品裁决 settle 契约归属（补服务端 or 明确该能力未交付）；B 需决定是否在集成 job 里 provision kind/k3s 集群（或明确改 T-05 策略，但那等于自削门禁）；C/D 待按 #1248 新日志继续分诊 | `main` 上该腿历史一直是 `skipped`（从未真正执行），README 原"CI 全绿"失实；本轮把"红"从不可归因收敛到可归因，剩余四项均已定性 |
