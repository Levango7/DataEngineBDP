...
V2.0 在 V1.0 云原生底座之上叠加五项云原生增强能力，使数擎大数据平台从"单集群可编排"进化为"多集群可治理、可灰度、可计量、可无服务器"的云原生数据平台：
...
# ============================================================================
# istiod Helm Chart 生产配置 - 数擎大数据平台 V2.0
# ============================================================================
...
- 问题：Cilium socketLB（socket-level load balancing）会绕过 Sidecar 拦截同节点 Pod 间流量，导致 Istio mTLS、可观测、灰度对同节点调用失效（仅跨节点流量走 Sidecar）。
- V2.0 方案：Cilium values 全局设置 `enableSocketLB: false`，Istio Sidecar 统一处理服务间流量（mTLS + 灰度 + 可观测）。Cilium 仅负责 NodeLocal 网络策略、NodePort/DNS 加速、可观测指标采集，不参与服务间流量治理。
- 备选方案：使用 Cilium 1.13+ 的 `cgroupv1` 兼容模式，或在 Istio 注入的 Namespace 关闭 socketLB（Namespace 级注解 `cilium.io/socketLB: "disabled"`）。
- 兼容矩阵：Cilium 1.13+ 与 Istio 1.20 的 socketLB 兼容性已验证（Cilium 官方兼容矩阵）。
...
- "秒级冷启"——函数从 0 副本冷启到可用约 3 秒（预热镜像 + 调度优化）。
...
| 冷启速度 | 约 3s（预热 + 调度优化） | 约 5s | Knative 胜出 |
...
伸缩流程：
1. **0→1（冷启）**：请求到达 Route，KPA 拉起 1 副本，约 3s（预热镜像 + 调度）。
...
# ============================================================================
# knative-serving Helm Chart 生产配置 - 数擎大数据平台 V2.0
# ============================================================================
...
| 风险 | 影响 | 对策 |
| --- | --- | --- |
| 冷启延迟 | 首请求等待 3s，体验下降 | 预热镜像 + activation-scale=1 + 节点池预留 |
...
# ============================================================================
# argocd Helm Chart 生产配置 - 数擎大数据平台 V2.0
# ============================================================================
...
V2.0 infra-orchestrator 扩展为多集群编排：

- **集群注册**：infra-orchestrator 创建新 SKE 集群后，自动注册到 Karmada（karmada-agent join）。
...
# ============================================================================
# karmada Helm Chart 生产配置 - 数擎大数据平台 V2.0
# ============================================================================
...
| 跨集群网络延迟 | 联邦查询慢 | 数据本地性下推 + 跨集群 join 仅小数据 + Istio mTLS 加速 |
...
| 资源项 | 计费单位 | 单价（元/月） | 计费方式 | 说明 |
| --- | --- | --- | --- | --- |
| CPU | 核·月 | 35 | 按用量 | 实际占用核数 × 时长 |
| 内存 | Gi·月 | 4 | 按用量 | 实际占用 Gi × 时长 |
| 存储 | Gi·月 | 0.3-0.5 | 按 StorageClass | JuiceFS 0.5，Ceph 0.3 |
| 网络 | GB | 0.5 | 按跨集群流量 | 集群内免费，跨集群计费 |
| GPU | 卡·月 | 5000 | 按用量 | 高单价，按需启用 |
...
# ============================================================================
# kubecost Helm Chart 生产配置 - 数擎大数据平台 V2.0
# ============================================================================
