...
- 灰度发布能力下沉到 Mesh 层，引擎升级（如 Trino 460→430）可按流量比例渐进切换，零中断。
...
spec:
  ...
  subsets:
    - name: stable
      labels: { version: v428 }      # 稳定版 Trino 460
    - name: canary
      labels: { version: v430 }      # 金丝雀版 Trino 460
```
...
| 标准版 | 0.85（85 折） | 基础 + 云原生增强 | 年付折扣 |
| 企业版 | 0.7（7 折） | 企业 + 专属支持 + 大规模 | 年付 + 大客户 |
