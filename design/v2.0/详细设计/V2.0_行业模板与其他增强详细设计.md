...
> V1.0 基线：industry-templates（Python/3 行业模板）、business-portal、open-api-catalog、asset-exchange、rule-engine（安全脱敏）

## 第1章 总体设计

### 1.1 V2.0 增强范围

V2.0 在 V1.0 基线之上扩展五大行业模板（金融、制造、零售、能源、政务），并将 V1.0 骨架组件（asset-exchange、open-api-catalog、rule-engine）演进为全流程落地能力，同时补齐安全合规、国密支持与可观测增强四项横切能力。

```mermaid
graph TB
    subgraph V1["V1.0 基线"]
        V11[industry-templates 3行业]
        V12[business-portal]
...
| --- | --- | --- | --- |
| 5 行业模板 | L5.3 | industry-templates（3 行业） | 扩展（新增 5 行业 + 增强现有 3 行业） |
...
2. **算法切换**：信创环境强制国密（`activeProvider=SM`），非信创环境可选国际算法。
3. **密钥轮换**：DEK 定期轮换（90 天），轮换后旧密文用旧 DEK 解密、新 DEK 重新加密。
4. **密钥安全**：KEK 存储于 KMS（硬件密码机或软件 KMS），永不离开 KMS。
5. **性能考虑**：SM4 性能与 AES 相当，SM2 性能优于 RSA，SM3 性能与 SHA-256 相当。
...
```mermaid
graph LR
    A[Phase 1<br/>5行业模板] --> B[Phase 2<br/>资产流通全流程]
...
| 阶段 | 内容 | 交付物 | 依赖 |
| --- | --- | --- | --- |
| Phase 1 | 5 行业模板 | 5 个 Helm Chart + DDL + DAG + Dashboard | V1.0 industry-templates |
...
| 模块 | 验收标准 | 验证方式 |
| --- | --- | --- |
| 5 行业模板 | 每个模板可安装/实例化/升级/卸载；DDL/DAG/Dashboard 正确 | 端到端测试 |
