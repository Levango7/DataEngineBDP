...
| --- | --- | --- | --- | --- | --- |
| 阶段 1 | V2.0.0-alpha | 复用 Trino 460 内置优化规则（谓词/投影下推）+ Calcite VolcanoPlanner 基础 | 谓词下推 + 投影下推 | alpha：可能产生次优计划 | 计划生成时间 P99 ≤ 500ms 需验证 |
...
| TrinoAdapter | Trino 460 | 全部下推 | 新增跨集群 Connector |
...
| FlinkAdapter | Flink 1.20 | 流式聚合下推 | 增强 Stream SQL 联邦 |
...
- **依赖**：Trino 460 SPI Connector 接口、Karmada 1.x（与 SKE K8s 版本兼容性见 §9.4）、远端 Shuffle Worker 镜像（自研，基于 Trino Worker 裁剪）。
...
| Trino | 417+（V2 读取） | Trino 460 | ✅ | V2 表读取、changelog 查询 | — |
| Doris | 2.0+（V2 External Catalog 读取） | Doris 2.1 | ✅ | V2 External Catalog 读取、物化视图基于 V2 表 | — |
| Flink | 1.15+（Iceberg V2 Sink） | Flink 1.20 | ✅ | V2 Sink、行级 UPSERT、changelog 消费 | — |
...
> 修复说明：Flink CDC 3.0（2024 年发布）要求 Flink 1.17+，V1.0 Flink 1.20 满足。但 Flink CDC 3.0 是较大版本升级（从 2.x 到 3.0 API 变更），V1.0 CDC 作业（基于 Flink CDC 2.x）迁移到 3.0 需重写作业 SQL/Connector 配置。本节给出兼容性验证结论与迁移指南。

##### 5.10.2.1 兼容性验证

表：Flink CDC 3.0 与 Flink 1.20 兼容性验证矩阵

| Flink CDC 版本 | 要求 Flink 版本 | V1.0 Flink 版本 | 兼容性 | 验证结论 |
| --- | --- | --- | --- | --- |
| Flink CDC 3.0.x | Flink 1.17+ | Flink 1.20 | ✅ 兼容 | MySQL-CDC/Postgres-CDC/Oracle-CDC 3.0 连接器在 Flink 1.20 运行通过，增量快照无锁全量验证通过 |
...
3. **L2.3 Flink**：复用 V1.0 Flink 1.20，流作业读写 Iceberg V2 表 changelog。
