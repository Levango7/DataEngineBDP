...
V2.0 在 V1.0 12 个 Executor Adapter 基础上，新增 IoTDB、Elasticsearch 两个联邦适配器，并增强现有适配器的下推能力。
...
| IoTDBAdapter | IoTDB 2.0 | 谓词/投影/降采样 | 新增，时序联邦查询 |
...
从源库 binlog 产生到 Iceberg 快照可见，端到端延迟目标 ≤ 10 秒。

```mermaid
flowchart LR
    B[binlog 产生] --> C[CDC 捕获<br/>延迟 1-2s]
    C --> F[Flink 处理<br/>延迟 2-3s]
    F --> W[写 Iceberg<br/>延迟 2-3s]
...
| --- | --- | --- | --- |
| 入仓端到端延迟 | 1-5min | ≤ 10s | binlog 到快照可见 |
...
| 质量校验延迟 | 5-60min | < 10s | 数据变更到质量校验完成 |
...
| 版本 | 重点 | 交付物 |
| --- | --- | --- |
| V2.0.0-alpha | 跨源联邦查询增强 + Flink CDC → Iceberg V2 | Calcite 联邦优化器 + CDC 管道 |
| V2.0.0-beta | 跨集群联邦 + 数据虚拟化 + 流批一体 | Karmada 集成 + 虚拟表 + 同表流批 |
| V2.0.0-rc | 实时 OLAP 物化视图自动同步 + 实时治理 | MV 自动刷新 + 事件总线治理 |
| V2.0.0 | 全模块 GA + SLA 达标 | 端到端秒级闭环 |
