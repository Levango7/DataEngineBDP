...
> **Token 映射**：`--ds-font-weight-bold` = 800（对应 ExtraBold，作为 bold 的标准值，用于页面标题 H1）。`--ds-font-weight-semibold` = 600、`--ds-font-weight-medium` = 500、`--ds-font-weight-normal` = 400。PageHeader.vue 通过 `var(--ds-font-weight-bold)` 引用，确保 H1 字重与规范一致。
