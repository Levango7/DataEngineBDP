...
> **Token 映射**：`--ds-font-weight-extrabold` = 800（对应 ExtraBold，用于页面标题 H1）；`--ds-font-weight-bold` = 700（对应 Bold，用于区块标题 H2 / 表格表头）。`--ds-font-weight-semibold` = 600、`--ds-font-weight-medium` = 500、`--ds-font-weight-normal` = 400。PageHeader.vue 通过 `var(--ds-font-weight-extrabold)` 引用 H1 字重，H2 / 表头通过 `var(--ds-font-weight-bold)` 引用，确保字重与规范一致。
...
| `--ds-radius-md-plus` | 10px | 中大圆角（历史 view 文件 10px 统一令牌化，`--radius-md-plus` 引用） |
| `--ds-radius-lg` | 12px | 卡片/按钮/输入框统一圆角（`--radius` 引用） |
| `--ds-radius-xl` | 16px | 大圆角 |
| `--ds-radius-2xl` | 20px | 胶囊圆角（环境标签） |
| `--ds-radius-full` | 9999px | 圆形（头像/圆点） |

> 禁止使用 6px 等非阶梯圆角；10px 应使用 `--ds-radius-md-plus` 令牌。§4.2 中"小圆角 6px"应迁移至 `--ds-radius-sm`（4px）或新增档位。
...
- 表头背景 `--c-surface`，字重 `var(--ds-font-weight-bold)`（700），字号 12px，颜色 `--ds-color-primary-900`（对应 `--typo-table-th` token）。
...
> **次级断点（1024px）**：部分组件级响应式（如内嵌面板、卡片网格、表格列折叠）使用 1024px 作为平板竖屏/横屏切换点。该断点不改变整体布局骨架，仅调整组件内部列数或面板折叠，与主断点 640/1440 不冲突。`design-tokens.css` 通过 `--ds-breakpoint-*` token 统一管理所有断点值。
