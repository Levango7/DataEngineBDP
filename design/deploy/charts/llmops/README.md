...
> LLMOps 平台 - 大模型训练部署运营 - 数据引擎大数据平台

## 1. 概述

`llmops` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `llmops` 组件。
...
```bash
# 安装到默认命名空间
helm install llmops design/deploy/charts/llmops -n sq-intelligent --create-namespace

# 使用自定义 values 覆盖
helm install llmops design/deploy/charts/llmops -n sq-intelligent \
...
```bash
helm upgrade llmops design/deploy/charts/llmops -n sq-intelligent \
  -f design/deploy/values/llmops-values.yaml
```
...
```bash
helm uninstall llmops -n sq-intelligent
```

### 3.4 渲染模板（Dry Run）

```bash
helm template llmops design/deploy/charts/llmops -n sq-intelligent
```

## 4. 配置参数

下表列出 `llmops` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `docker.m.daocloud.io/sq-llmops` |
...
# llmops 自定义配置示例
image:
  repository: "docker.m.daocloud.io/sq-llmops"
  tag: "0.1.0"
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=llmops" -n sq-intelligent
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=llmops" -n sq-intelligent -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/llmops 8080:8080 -n sq-intelligent
```

### 7.4 获取当前配置

```bash
helm get values llmops -n sq-intelligent
```
...
```bash
# 查看历史版本
helm history llmops -n sq-intelligent

# 回滚到上一版本
helm rollback llmops -n sq-intelligent
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/llmops-values.yaml`
