...
> Keycloak 身份认证与访问管理 - 数据引擎大数据平台

## 1. 概述

`keycloak` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `keycloak` 组件。
...
```bash
# 安装到默认命名空间
helm install keycloak design/deploy/charts/keycloak -n default --create-namespace

# 使用自定义 values 覆盖
helm install keycloak design/deploy/charts/keycloak -n default \
  -f design/deploy/values/keycloak-values.yaml
```

### 3.2 升级

```bash
helm upgrade keycloak design/deploy/charts/keycloak -n default \
  -f design/deploy/values/keycloak-values.yaml
```
...
```bash
helm template keycloak design/deploy/charts/keycloak -n default
```

## 4. 配置参数

下表列出 `keycloak` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `sq-keycloak` |
| `image.tag` | 镜像标签 | `24.0-0.1.0` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=keycloak" -n default
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=keycloak" -n default -f
```
...
```bash
helm get values keycloak -n default
```
...
```bash
# 查看历史版本
helm history keycloak -n default

# 回滚到上一版本
helm rollback keycloak -n default
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/keycloak-values.yaml`
