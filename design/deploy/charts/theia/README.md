...
> Eclipse Theia Web IDE - 数据引擎大数据平台数据开发IDE

## 1. 概述

`theia` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `theia` 组件。
...
```bash
# 安装到默认命名空间
helm install theia design/deploy/charts/theia -n default --create-namespace

# 使用自定义 values 覆盖
helm install theia design/deploy/charts/theia -n default \
  -f design/deploy/values/theia-values.yaml
```

### 3.2 升级

```bash
helm upgrade theia design/deploy/charts/theia -n default \
  -f design/deploy/values/theia-values.yaml
```
...
```bash
helm uninstall theia -n default
```

### 3.4 渲染模板（Dry Run）

```bash
helm template theia design/deploy/charts/theia -n default
```

## 4. 配置参数

下表列出 `theia` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `sq-theia` |
| `image.tag` | 镜像标签 | `1.45-0.1.0` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=theia" -n default
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=theia" -n default -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/theia 3000:3000 -n default
```

### 7.4 获取当前配置

```bash
helm get values theia -n default
```
...
```bash
# 查看历史版本
helm history theia -n default

# 回滚到上一版本
helm rollback theia -n default
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/theia-values.yaml`
