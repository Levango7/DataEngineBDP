...
> Reloader - ConfigMap/Secret 变更自动滚动重启 - 数据引擎大数据平台

## 1. 概述

`reloader` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `reloader` 组件。
...
```bash
# 安装到默认命名空间
helm install reloader design/deploy/charts/reloader -n reloader --create-namespace

# 使用自定义 values 覆盖
helm install reloader design/deploy/charts/reloader -n reloader \
  -f design/deploy/values/reloader-values.yaml
```

### 3.2 升级

```bash
helm upgrade reloader design/deploy/charts/reloader -n reloader \
  -f design/deploy/values/reloader-values.yaml
```
...
```bash
helm template reloader design/deploy/charts/reloader -n reloader
```

## 4. 配置参数

下表列出 `reloader` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `ghcr.io/stakater/reloader` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=reloader" -n reloader
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=reloader" -n reloader -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/reloader 8080:8080 -n reloader
```

### 7.4 获取当前配置

```bash
helm get values reloader -n reloader
```
...
```bash
# 查看历史版本
helm history reloader -n reloader

# 回滚到上一版本
helm rollback reloader -n reloader
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/reloader-values.yaml`
