...
> Velero 备份恢复 - Kubernetes 集群与 PV 灾备 - 数据引擎大数据平台

## 1. 概述

`velero` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `velero` 组件。
...
```bash
# 安装到默认命名空间
helm install velero design/deploy/charts/velero -n velero --create-namespace

# 使用自定义 values 覆盖
helm install velero design/deploy/charts/velero -n velero \
  -f design/deploy/values/velero-values.yaml
```

### 3.2 升级

```bash
helm upgrade velero design/deploy/charts/velero -n velero \
  -f design/deploy/values/velero-values.yaml
```
...
```bash
helm template velero design/deploy/charts/velero -n velero
```

## 4. 配置参数

下表列出 `velero` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `velero/velero` |
| `image.tag` | 镜像标签 | `v1.13.0` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=velero" -n velero
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=velero" -n velero -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/velero 8085:8085 -n velero
```

### 7.4 获取当前配置

```bash
helm get values velero -n velero
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/velero-values.yaml`
