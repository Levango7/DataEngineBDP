...
> FinOps 计费分析 - 多租户成本归集与计费出账 - 数据引擎大数据平台

## 1. 概述

`finops` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `finops` 组件。
...
```bash
# 安装到默认命名空间
helm install finops design/deploy/charts/finops -n sq-system --create-namespace

# 使用自定义 values 覆盖
helm install finops design/deploy/charts/finops -n sq-system \
  -f design/deploy/values/finops-values.yaml
```

### 3.2 升级

```bash
helm upgrade finops design/deploy/charts/finops -n sq-system \
  -f design/deploy/values/finops-values.yaml
```
...
```bash
helm uninstall finops -n sq-system
```

### 3.4 渲染模板（Dry Run）

```bash
helm template finops design/deploy/charts/finops -n sq-system
```

## 4. 配置参数

下表列出 `finops` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `harbor.shuqing.io/shuqing/finops` |
...
# finops 自定义配置示例
image:
  repository: "harbor.shuqing.io/shuqing/finops"
  tag: "2.0.0"
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=finops" -n sq-system
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=finops" -n sq-system -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/finops 8080:8080 -n sq-system
```

### 7.4 获取当前配置

```bash
helm get values finops -n sq-system
```
...
```bash
# 查看历史版本
helm history finops -n sq-system

# 回滚到上一版本
helm rollback finops -n sq-system
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/finops-values.yaml`
