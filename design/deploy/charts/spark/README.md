...
> Apache Spark 计算引擎 - 数据引擎大数据平台

## 1. 概述

`spark` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `spark` 组件。
...
```bash
# 安装到默认命名空间
helm install spark design/deploy/charts/spark -n default --create-namespace

# 使用自定义 values 覆盖
helm install spark design/deploy/charts/spark -n default \
...
```bash
helm upgrade spark design/deploy/charts/spark -n default \
  -f design/deploy/values/spark-values.yaml
```
...
```bash
helm uninstall spark -n default
```

### 3.4 渲染模板（Dry Run）

```bash
helm template spark design/deploy/charts/spark -n default
```

## 4. 配置参数

下表列出 `spark` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `sq-spark` |
| `image.tag` | 镜像标签 | `3.5-0.1.0` |
| `image.pullPolicy` | 镜像拉取策略 | `IfNotPresent` |
| `replicaCount` | 副本数 | `1` |
| `service.type` | Service 类型 | `ClusterIP` |
| `service.port` | Service 端口 | `4040` |
| `service.containerPort` | 容器端口 | `4040` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=spark" -n default
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=spark" -n default -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/spark 4040:4040 -n default
```

### 7.4 获取当前配置

```bash
helm get values spark -n default
```
...
```bash
# 查看历史版本
helm history spark -n default

# 回滚到上一版本
helm rollback spark -n default
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/spark-values.yaml`
