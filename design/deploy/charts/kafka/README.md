...
> Apache Kafka 消息中间件 - 数据引擎大数据平台

## 1. 概述

`kafka` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `kafka` 组件。
...
```bash
# 安装到默认命名空间
helm install kafka design/deploy/charts/kafka -n default --create-namespace

# 使用自定义 values 覆盖
helm install kafka design/deploy/charts/kafka -n default \
  -f design/deploy/values/kafka-values.yaml
```

### 3.2 升级

```bash
helm upgrade kafka design/deploy/charts/kafka -n default \
  -f design/deploy/values/kafka-values.yaml
```
...
```bash
helm template kafka design/deploy/charts/kafka -n default
```

## 4. 配置参数

下表列出 `kafka` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `sq-kafka` |
| `image.tag` | 镜像标签 | `3.7-0.1.0` |
| `image.pullPolicy` | 镜像拉取策略 | `IfNotPresent` |
| `replicaCount` | 副本数 | `1` |
| `service.type` | Service 类型 | `ClusterIP` |
| `service.port` | Service 端口 | `9092` |
| `service.containerPort` | 容器端口 | `9092` |
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=kafka" -n default
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=kafka" -n default -f
```
...
```bash
helm get values kafka -n default
```
...
```bash
# 查看历史版本
helm history kafka -n default

# 回滚到上一版本
helm rollback kafka -n default
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/kafka-values.yaml`
