...
> dqctl CLI - 数据质量运维命令行 - 数据引擎大数据平台

## 1. 概述

`dqctl` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `dqctl` 组件。
...
```bash
# 安装到默认命名空间
helm install dqctl design/deploy/charts/dqctl -n sq-governance --create-namespace

# 使用自定义 values 覆盖
helm install dqctl design/deploy/charts/dqctl -n sq-governance \
  -f design/deploy/values/dqctl-values.yaml
```

### 3.2 升级

```bash
helm upgrade dqctl design/deploy/charts/dqctl -n sq-governance \
  -f design/deploy/values/dqctl-values.yaml
```
...
```bash
helm uninstall dqctl -n sq-governance
```

### 3.4 渲染模板（Dry Run）

```bash
helm template dqctl design/deploy/charts/dqctl -n sq-governance
```

## 4. 配置参数

下表列出 `dqctl` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `docker.m.daocloud.io/sq-dqctl` |
...
# dqctl 自定义配置示例
image:
  repository: "docker.m.daocloud.io/sq-dqctl"
  tag: "0.1.0"
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=dqctl" -n sq-governance
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=dqctl" -n sq-governance -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/dqctl 8080:8080 -n sq-governance
```

### 7.4 获取当前配置

```bash
helm get values dqctl -n sq-governance
```
...
```bash
# 查看历史版本
helm history dqctl -n sq-governance

# 回滚到上一版本
helm rollback dqctl -n sq-governance
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/dqctl-values.yaml`
