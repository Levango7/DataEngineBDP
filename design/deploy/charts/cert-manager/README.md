...
> 证书管理控制器 - 自动化 TLS 证书签发与轮转 - 数据引擎大数据平台

## 1. 概述

`cert-manager` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `cert-manager` 组件。
...
```bash
# 安装到默认命名空间
helm install cert-manager design/deploy/charts/cert-manager -n cert-manager --create-namespace

# 使用自定义 values 覆盖
helm install cert-manager design/deploy/charts/cert-manager -n cert-manager \
  -f design/deploy/values/cert-manager-values.yaml
```

### 3.2 升级

```bash
helm upgrade cert-manager design/deploy/charts/cert-manager -n cert-manager \
  -f design/deploy/values/cert-manager-values.yaml
```
...
```bash
helm template cert-manager design/deploy/charts/cert-manager -n cert-manager
```

## 4. 配置参数

下表列出 `cert-manager` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `quay.io/jetstack/cert-manager-controller` |
| `image.tag` | 镜像标签 | `v1.14.0` |
| `image.pullPolicy` | 镜像拉取策略 | `IfNotPresent` |
| `replicaCount` | 副本数 | `1` |
| `service.type` | Service 类型 | `ClusterIP` |
| `service.port` | Service 端口 | `9402` |
| `service.containerPort` | 容器端口 | `9402` |
...
ingress:
  ...
  className: apisix
  hosts:
    - host: cert-manager.shuqing.local
      paths:
        - path: /
          pathType: Prefix
```
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=cert-manager" -n cert-manager
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=cert-manager" -n cert-manager -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/cert-manager 9402:9402 -n cert-manager
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/cert-manager-values.yaml`
