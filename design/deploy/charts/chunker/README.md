...
> 数据分块服务 - 大文件分块存储与并行处理 - 数据引擎大数据平台

## 1. 概述

`chunker` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `chunker` 组件。
...
```bash
# 安装到默认命名空间
helm install chunker design/deploy/charts/chunker -n sq-engine --create-namespace

# 使用自定义 values 覆盖
helm install chunker design/deploy/charts/chunker -n sq-engine \
  -f design/deploy/values/chunker-values.yaml
```

### 3.2 升级

```bash
helm upgrade chunker design/deploy/charts/chunker -n sq-engine \
  -f design/deploy/values/chunker-values.yaml
```
...
```bash
helm uninstall chunker -n sq-engine
```

### 3.4 渲染模板（Dry Run）

```bash
helm template chunker design/deploy/charts/chunker -n sq-engine
```

## 4. 配置参数

下表列出 `chunker` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `harbor.shuqing.io/shuqing/chunker` |
...
# chunker 自定义配置示例
image:
  repository: "harbor.shuqing.io/shuqing/chunker"
  tag: "2.0.0"
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=chunker" -n sq-engine
```

### 7.2 查看日志

```bash
kubectl logs -l "app.kubernetes.io/instance=chunker" -n sq-engine -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/chunker 8080:8080 -n sq-engine
```

### 7.4 获取当前配置

```bash
helm get values chunker -n sq-engine
```
...
```bash
# 查看历史版本
helm history chunker -n sq-engine

# 回滚到上一版本
helm rollback chunker -n sq-engine
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/chunker-values.yaml`
