...
> External Secrets - 外部密钥管理系统集成 - 数据引擎大数据平台

## 1. 概述

`external-secrets` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `external-secrets` 组件。

- **Chart 版本**：`2.0.0`
- **App 版本**：`2.0.0`
- **默认命名空间**：`external-secrets`
...
```bash
# 安装到默认命名空间
helm install external-secrets design/deploy/charts/external-secrets -n external-secrets --create-namespace

# 使用自定义 values 覆盖
helm install external-secrets design/deploy/charts/external-secrets -n external-secrets \
  -f design/deploy/values/external-secrets-values.yaml
```

### 3.2 升级

```bash
helm upgrade external-secrets design/deploy/charts/external-secrets -n external-secrets \
  -f design/deploy/values/external-secrets-values.yaml
```
...
```bash
helm template external-secrets design/deploy/charts/external-secrets -n external-secrets
```

## 4. 配置参数

下表列出 `external-secrets` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `ghcr.io/external-secrets/external-secrets` |
| `image.tag` | 镜像标签 | `v0.9.0` |
...
ingress:
  ...
  className: apisix
  hosts:
    - host: external-secrets.shuqing.local
      paths:
        - path: /
          pathType: Prefix
```
...
```bash
kubectl logs -l "app.kubernetes.io/instance=external-secrets" -n external-secrets -f
```

### 7.3 端口转发（本地调试）

```bash
kubectl port-forward svc/external-secrets 8080:8080 -n external-secrets
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/external-secrets-values.yaml`
