...
> NGINX Ingress 控制器 - 南北向流量入口与负载均衡 - 数据引擎大数据平台

## 1. 概述

`ingress-nginx` 是数据引擎大数据平台（DataEngineBDP）的 Helm Chart，用于在 Kubernetes 集群上一键部署和管理 `ingress-nginx` 组件。

- **Chart 版本**：`2.0.0`
- **App 版本**：`2.0.0`
- **默认命名空间**：`ingress-nginx`
...
```bash
# 安装到默认命名空间
helm install ingress-nginx design/deploy/charts/ingress-nginx -n ingress-nginx --create-namespace

# 使用自定义 values 覆盖
helm install ingress-nginx design/deploy/charts/ingress-nginx -n ingress-nginx \
  -f design/deploy/values/ingress-nginx-values.yaml
```

### 3.2 升级

```bash
helm upgrade ingress-nginx design/deploy/charts/ingress-nginx -n ingress-nginx \
  -f design/deploy/values/ingress-nginx-values.yaml
```
...
```bash
helm template ingress-nginx design/deploy/charts/ingress-nginx -n ingress-nginx
```

## 4. 配置参数

下表列出 `ingress-nginx` Chart 的主要配置参数，完整配置参见 `values.yaml`。

| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `image.repository` | 镜像仓库地址 | `registry.k8s.io/ingress-nginx/controller` |
...
ingress:
  ...
  className: apisix
  hosts:
    - host: ingress-nginx.shuqing.local
      paths:
        - path: /
          pathType: Prefix
```
...
```bash
kubectl get pods -l "app.kubernetes.io/instance=ingress-nginx" -n ingress-nginx
```
...
- **Chart 版本**：2.0.0
- **App 版本**：2.0.0
- **关联配置**：`design/deploy/values/ingress-nginx-values.yaml`
