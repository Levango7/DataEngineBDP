...
> 关联：`design/deploy/README.md`；`design/deploy/DEPLOY_GUIDE.md`；`design/deploy/MULTI_ARCH_BUILD.md`；`design/deploy/JWT鉴权配置指南.md`（AUTH_MODE fail-fast 配套）；`design/deploy/向量检索embedding联调指南.md`（vector-engine 语义检索启用）；`design/部署/灰度发布方案.md`；`design/运维/运维手册.md`
