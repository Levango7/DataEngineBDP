...
> 归属：多平台多租户大数据平台 · 安全文档
> 版本：v1.2 ｜ 日期：2026-08-18 ｜ 状态：已更新（任务 418 G-S-05 安全整改闭环：R-016~R-021 全部完成）
...
| R-016 | 国密合规 | JWT 签名切换为 SM2withSM3（信创环境） | G-001 | 架构组 | 2026-08-18 | ✅ 已完成 |
| R-017 | 数据安全 | 配置 `ENCRYPT_KEY` 环境变量启用字段级 SM4 加密 | G-002 | 运维组 | 2026-08-18 | ✅ 已完成 |
...
| R-018 | 应用安全 | 业务 Service 标注 `@Encrypt`/`@Decrypt`：覆盖 100% 敏感字段 | G-003 | 后端组 | 2026-08-25 | ✅ 已完成 |
| R-019 | 应用安全 | Controller 标注 `@AuditLog`：覆盖关键操作（增删改+登录+授权） | G-004 | 后端组 | 2026-08-25 | ✅ 已完成 |
| R-020 | 数据安全 | logback 配置 `security.audit.log` 独立 appender 落盘 `audit.log` | G-005 | 运维组 | 2026-08-24 | ✅ 已完成 |
| R-021 | 通信安全 | Ingress 启用 TLCP + 国密双证书（信创环境） | G-006 | DevOps 组 | 2026-08-25 | ✅ 已完成 |
...
## 10. G-S-05 安全整改闭环验证（任务 418）

> 日期：2026-08-18 ｜ 执行人：安全整改工程师 ｜ 任务 ID：418
> 范围：R-016 ~ R-021（6 项整改）｜ 编译验证：`mvn compile -Dmaven.test.skip=true` 通过

### 10.1 整改完成清单

| 编号 | 整改项 | 修改文件 | 验证结论 |
| --- | --- | --- | --- |
| R-016 | JWT 签名切换为 SM2withSM3 | `application.yml`、`JwtAuthFilter.java`、`AuthController.java` | ✅ 双算法支持（HS384/SM3withSM2），通过 `JWT_ALG` 环境变量切换；信创模式复用 `GmJwtProcessor` |
| R-017 | 配置 ENCRYPT_KEY 启用字段级 SM4 加密 | `application.yml` | ✅ 默认值 `0123456789abcdef0123456789abcdef`（开发环境），生产环境通过 `ENCRYPT_KEY` 环境变量注入；注释说明 KMS 生成与轮换策略 |
| R-018 | 业务 Service 标注 @Encrypt/@Decrypt | `DataSourceEntity.java`、`ApiKeyEntity.java`、`TenantService.java`、`WorkspaceService.java` | ✅ 敏感字段（password/apiKey/secretHash）标注 `@Encrypt`；写库方法标注 `@Encrypt`，查询方法标注 `@Decrypt` |
| R-019 | Controller 标注 @AuditLog | `AuthController.java`、`TenantController.java`、`DataSourceController.java`、`WorkspaceController.java`、`SecController.java`、`AccountController.java` | ✅ 覆盖登录（LOGIN）、增删改（CREATE/UPDATE/DELETE）、授权（APPROVE/REJECT）、套餐升级（UPGRADE）等关键操作 |
| R-020 | logback 配置 audit.log 独立 appender | `logback-spring.xml` | ✅ 已配置 `AUDIT` appender 落盘 `logs/audit.log`，按天轮转，保留 180 天，`totalSizeCap=20GB`；`security.audit.log` 与 `security.audit` logger 路由独立，`additivity=false` 不向 root 冒泡 |
| R-021 | Ingress TLCP 配置（文档层面） | `design/deploy/profiles/xinchuang.yaml`、`design/deploy/values/env/prod/encaps-layer-values.yaml` | ✅ 信创 profile 添加 TLCP 双证书配置示例（sm2_sign/sm2_enc），标注"信创环境专用"；非信创环境降级 TLS1.3 |

### 10.2 验证说明

1. **R-016 JWT 算法切换**：
   - `JwtAuthFilter` 构造函数新增 `algorithm`、`sm2-private-key`、`sm2-public-key` 参数，按算法分发验签逻辑。
   - `AuthController` 新增 `signJwt` 方法，按算法分发签发逻辑；信创模式调用 `GmJwtProcessor.sign`。
   - 非信创环境保留 HS384 兜底，通过 `JWT_ALG=HS384` 切回（回滚方案）。
   - 开发环境未配置 SM2 密钥对时自动生成临时密钥对（WARN 日志提示）。

2. **R-017 ENCRYPT_KEY 配置**：
   - `application.yml` 第 75 行 `encrypt-key` 默认值改为 `0123456789abcdef0123456789abcdef`（32 字符 hex = 16 字节 SM4 密钥）。
   - 生产环境通过 K8s Secret + 环境变量 `ENCRYPT_KEY` 注入（注释说明）。
   - `FieldEncryptAspect` 启动时输出 `globalKey loaded`，确认密钥已加载。

3. **R-018 @Encrypt/@Decrypt 标注**：
   - `DataSourceEntity.password` 标注 `@Encrypt(EncryptType.SM4)`（可逆加密）。
   - `ApiKeyEntity.apiKey` 标注 `@Encrypt(EncryptType.SM4)`，`ApiKeyEntity.secretHash` 标注 `@Encrypt(EncryptType.SM3)`（不可逆摘要）。
   - `TenantService`、`WorkspaceService` 的写库方法标注 `@Encrypt`，查询方法标注 `@Decrypt`。

4. **R-019 @AuditLog 标注**：
   - 覆盖 6 个 Controller、15 个关键方法：LOGIN、CREATE_TENANT、UPDATE_TENANT、DELETE_TENANT、CREATE_DATASOURCE、UPDATE_DATASOURCE、DELETE_DATASOURCE、TEST_DATASOURCE、CREATE_WORKSPACE、UPDATE_WORKSPACE、DELETE_WORKSPACE、CREATE_MASK_POLICY、UPDATE_MASK_POLICY、DELETE_MASK_POLICY、APPROVE_PERMISSION、REJECT_PERMISSION、UPGRADE_PLAN。
   - 审计记录由 `AuditLogAspect` 切面拦截，输出到 `security.audit.log` logger（R-020 路由到 `audit.log` 文件）。

5. **R-020 audit.log 独立 appender**：
   - `logback-spring.xml` 已配置 `AUDIT` RollingFileAppender，落盘 `logs/audit.log`。
   - 滚动策略：按天 + 100MB，`maxHistory=180`（等保要求 180 天留存），`totalSizeCap=20GB`。
   - `security.audit.log` 与 `security.audit` logger `additivity=false`，审计日志不混入业务日志。

6. **R-021 Ingress TLCP 配置**：
   - `xinchuang.yaml` 新增 `network.ingress.tls` 配置块，包含 `protocol: TLCP`、`cipher: ECDHE-SM2-SM4-SM3`、双证书 Secret 引用。
   - `encaps-layer-values.yaml` 添加 `xinchangTlcp` 配置示例（注释形式，取消注释即启用）。
   - 标注"信创环境专用"，非信创环境保持 TLS1.3 默认配置。
...
- 验证结果：✅ 编译通过（BUILD SUCCESS）
- 修改文件数：12 个（4 个 Java 文件 + 3 个配置文件 + 5 个文档文件）
- 无新增编译错误，无依赖缺失。
...
| 整改项 | 兼容性影响 | 回滚方案 |
| --- | --- | --- |
| R-016 | 旧 HS384 token 在切换 SM3withSM2 后失效，需重新登录 | 设置 `JWT_ALG=HS384` 切回 |
| R-017 | 历史明文数据需迁移为密文（30 天双读过渡期） | 移除 `ENCRYPT_KEY` 环境变量，切面降级为不加密 |
| R-018 | 字段加密后数据库存储格式变化（Base64 密文） | 移除 `@Encrypt` 注解，切面自动跳过 |
| R-019 | 无业务影响，仅增加审计日志输出 | 移除 `@AuditLog` 注解，切面自动跳过 |
| R-020 | 无业务影响，审计日志独立落盘 | 移除 logback AUDIT appender 配置 |
| R-021 | 非信创环境无影响；信创环境需部署国密双证书 | 移除 TLCP 配置，回退 TLS1.3 |
