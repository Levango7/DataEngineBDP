...
数据引擎大数据平台 V2.0.0 GA 是 V1.0.0 的重大升级版本，代号 **Aurora（极光）**。本版本在 V1.0.0 已交付的 21 个自研组件、59 个 Helm Chart、43 份设计文档的基础上，围绕 **云原生、AI 能力、数据联邦、实时数仓、行业模板、安全合规** 六大方向进行系统化增强，并最终通过等保三级测评与密评，形成可正式投入生产的 GA 版本。
...
- **数据联邦与实时数仓**：手写 SQL 解析 + 跨源归并引擎驱动的跨源 Join，Flink CDC + Iceberg V2 upsert + Doris 物化视图构成的实时数仓闭环。
...
- **手写 SQL 解析 + 跨源归并引擎**：SQL 网关基于自研手写 SQL 解析器 + 跨源归并引擎实现统一查询优化，支持基于成本的查询改写、谓词下推、投影裁剪、Join 重排（Apache Calcite 集成规划中）。
...
- **查询改写策略**：自研跨源归并引擎新增 12 种查询改写规则，包括视图合并、子查询去关联、聚合下推、Join 重排等。
...
| sql-gateway | 1.0.0 | 2.0.0 | Java | 手写 SQL 解析 + 跨源归并引擎，跨源 Join |
