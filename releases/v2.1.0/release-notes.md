...
| 上一个版本 | V2.0.0-RC（2026-08-08，代号 Aurora，GA 定级已修订为 RC，见 ERRATUM.md） |
...
| 兼容性 | 与 V2.0.0-RC 完全兼容，平滑升级，无破坏性变更 |

---

## 2. 发布概述

数据引擎大数据平台 V2.1.0 是 V2.0.0-RC（Aurora，GA 定级已修订为 RC）之后的特性增强版本，代号 **Borealis（北极光）**。本版本聚焦三大主线交付：
...
3. **v2.1 行业生态扩展**：在 V2.0.0 已交付的金融、能源、政务、零售、制造 5 个行业模板骨架基础上，新增医疗、交通、教育、农牧 4 个行业模板，行业覆盖扩展至 9 个（口径：3 套基础模板已实现 + 6 套骨架已交付）；同时引入 Argo Rollouts 金丝雀渐进式交付能力。
...
- **性能可感知**：SQL 网关查询结果缓存（Caffeine 60s TTL + 租户隔离键）、K8s informer watch 缓存、规则引擎异步批量执行（parallelStream + 失败隔离）、88 个 Chart HPA autoscaling，平台具备生产级性能与弹性能力。
...
| 88 个 Chart HPA autoscaling | 为全部 88 个 Helm Chart 配置 HPA autoscaling，支持基于 CPU / 内存 / 自定义指标的弹性伸缩，生产环境资源利用率提升 40%+ | `32f6412` |
...
V2.0.0-RC 的端到端链路在 V2.1.0 中全部完成真实组件联调与 E2E 验证，形成可重复执行的验证基线。
...
在 V2.0.0 已交付的金融、能源、政务、零售、制造 5 个行业模板骨架基础上，新增 4 个行业模板，行业覆盖扩展至 9 个（口径：3 套基础模板已实现 + 6 套骨架已交付）。所有新模板位于 `platform/industry-templates/industry_templates/templates/`。
...
| 集群弹性伸缩 | 手动 / 定时 | HPA 自动 | 利用率 40%+ ↑ | 88 Chart HPA autoscaling |
...
V2.1.0 与 V2.0.0-RC **完全兼容**，无破坏性变更，可平滑升级：

- **API 兼容**：V2.0.0-RC 的全部 API 在 V2.1.0 中保留且行为一致。
- **配置兼容**：V2.0.0-RC 的 Helm Values 在 V2.1.0 中可直接使用，新增配置项均有默认值。
...
| 角色 | 签字 | 日期 |
| --- | --- | --- |
| 架构负责人 | _______________ | 2026-08-27 |
| 开发负责人 | _______________ | 2026-08-27 |
