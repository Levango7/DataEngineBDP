...
Start-Job -Name "build-llm-gateway" -ScriptBlock {
    ...
    $code = $LASTEXITCODE
    $output | Out-File "F:\Agent\workbuddy\workspace\DataEngineBDP\deploy\k3s\llm-gateway-build.log"
...
Start-Job -Name "build-baremetal" -ScriptBlock {
    ...
    $code = $LASTEXITCODE
    $output | Out-File "F:\Agent\workbuddy\workspace\DataEngineBDP\deploy\k3s\baremetal-build.log"
