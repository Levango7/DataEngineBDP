      ...
      <el-row :gutter="12" role="list" aria-label="大数据组件状态列表">
        <el-col v-for="comp in components" :key="comp.name" :xs="12" :sm="8" :md="6" :lg="4">
          <div
            ...
            :class="comp.status"
            role="listitem"
            :aria-label="`组件 ${comp.name} 状态：${compStatusLabel(comp.status)}`"
          >
      ...
      >
        ...
        <el-tab-pane label="网络配置" name="network">
          ...
          <div v-else-if="networkError" class="tab-error" role="alert">
            加载失败：{{ networkError.message }}
          </div>
          <div v-else-if="networkConfig">
            <el-descriptions :column="4" border size="small" style="margin-bottom: 12px">
              <el-descriptions-item label="Pod CIDR">
            ...
            <el-table
              ...
              border
              role="table"
              aria-label="NetworkPolicy 列表"
              empty-text="暂无 NetworkPolicy"
            >
            ...
            <h4 style="margin-top: 12px">
              Service 列表（{{ networkConfig.services?.length ?? 0 }}）
            </h4>
            <el-table
            ...
            <h4 style="margin-top: 12px">
              Ingress 列表（{{ networkConfig.ingresses?.length ?? 0 }}）
            </h4>
            <el-table
              ...
              border
              role="table"
              aria-label="Ingress 列表"
              empty-text="暂无 Ingress"
            >
              <el-table-column prop="name" label="名称" min-width="120" />
              <el-table-column prop="namespace" label="命名空间" min-width="100" />
        ...
        <el-tab-pane label="存储配置" name="storage">
          ...
          <div v-else>
            ...
            <el-table
              ...
              border
              role="table"
              aria-label="StorageClass 列表"
              empty-text="暂无 StorageClass"
            >
              <el-table-column prop="name" label="名称" min-width="120" />
        ...
        <el-tab-pane label="HPA 自动伸缩" name="hpa">
          ...
          <el-table
            ...
            border
            role="table"
            aria-label="HPA 自动伸缩策略列表"
            empty-text="暂无 HPA 策略"
          >
            <el-table-column prop="name" label="名称" min-width="120" />
            <el-table-column prop="namespace" label="命名空间" min-width="100" />
